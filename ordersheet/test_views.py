...
class ordersheetViewsTests(TestCase):
    ...
    fixtures = ['May23datadump.json', 'categories.json']

    def test_ordersheet_list_view(self):
        resp = self.client.get(reverse('ordersheet'))
        self.assertEqual(resp.status_code, 200)
        self.assertTemplateUsed(resp, template_name='ordersheet/ordersheet.html')
    ...
    def test_add_item_qty_size_to_add_ordersheet_views(self):
        resp = self.client.post(
            '/ordersheet/add/1/', {'quantity': '7', 'redirect_url': 'ordersheet'})
        messages = list(get_messages(resp.wsgi_request))
        self.assertEqual(resp.status_code, 302)
        self.assertEqual(len(messages), 1)
        self.assertEqual(str(messages[0]),
                         'Added Information Security Policy to your ordersheet')

    def test_add_item_qty_to_add_ordersheet_views(self):
        resp = self.client.post(
            '/ordersheet/add/2/', {'quantity': '4', 'redirect_url': 'ordersheet', })
        messages = list(get_messages(resp.wsgi_request))
        self.assertEqual(resp.status_code, 302)
        self.assertEqual(len(messages), 1)
        self.assertEqual(str(messages[0]),
